...
import { BIZ_PHONE, BIZ_TEL_HREF, SITE_URL } from "@/lib/site";
...
export default function Booking({ dateModified }: { dateModified: string }) {
  const howto = {
    ...
    description: "전화 한 통으로 자리부터 부킹까지 끝내는 4단계 가이드",
    totalTime: "PT2M",
    ...
    <>
      ...
      <header className="hero hero-sub">
        <div className="hero-inner">
          ...
          <h1>전화 한 통이면 <span className="grad">자리·부킹·룸</span>이 끝납니다.</h1>
          <p className="lead">매니저 짱구 직통 <strong>{BIZ_PHONE}</strong>. 인원·성비·콘셉트만 알려주시면 그날 분위기에 맞는 자리로 매칭됩니다.</p>
          <div className="cta">
            <a className="btn btn-primary btn-lg" href={BIZ_TEL_HREF}>📞 지금 바로 직통</a>
      ...
      <main className="wrap">
        ...
        <section className="ps">
          <h3>저장해두면 편한 번호</h3>
          <p>매니저 짱구 직통 <a href={BIZ_TEL_HREF}>{BIZ_PHONE}</a>. 단축번호 등록해두시면 출발 전 한 번에 끝납니다.</p>
        </section>
      </main>
